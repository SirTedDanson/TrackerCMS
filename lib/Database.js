...
class Database {
  ...
  viewRoles() {
    return db.promise().query(`
      SELECT role.id, role.title AS job_title, role.salary, department.name AS department
      FROM role
  ...
  viewEmployees() {
    return db.promise().query(`
      SELECT employee.id, employee.first_name, employee.last_name, role.title AS job_title, department.name AS department, role.salary, employee.manager_id AS manager
      FROM employee
  ...
  addEmployee(id, firstName, lastName) {
    ...
    VALUES (?,?,?,?)`;
    const params = [firstName, lastName, id, null]; 
    return db.promise().query(sql, params)
      .then(([rows, fields]) => {
